...
from Pixsuvy.helpers.tools import get_arg, get_text, resize_media
from Pixsuvy.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    pix = await edit_or_reply(message, "`It's okay if the sticker is hollow...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await pix.edit("**The sticker has no Name!**")
                return
            ...
            await pix.edit("**Unsupported Files**")
            return
        media_ = await client.download_media(replied, file_name="Pixsuvy/resources/")
    else:
        await pix.edit("**Please Reply to Photo/GIF/Sticker Media!**")
        return
    if media_:
        ...
        if exist is not False:
            try:
                ...
                return await pix.edit(f"**ERROR:** `{e}`")
            ...
            await pix.edit("`Create a New Sticker Pack`")
            try:
                await client.send_message("Stickers", cmd)
            except YouBlockedUser:
                await client.unblock_user("stickers")
...
async def tinying(client: Client, message: Message):
    ...
    if not (reply and (reply.media)):
        return await edit_or_reply(message, "**Please Reply To Sticker Message!**")
    pix = await edit_or_reply(message, "`Processing . . .`")
    ik = await client.download_media(reply)
    im1 = Image.open("Pixsuvy/resources/blank.png")
    if ik.endswith(".tgs"):
        await client.download_media(reply, "pix.tgs")
        await bash("lottie_convert.py pix.tgs json.json")
        json = open("json.json", "r")
        jsn = json.read()
        jsn = jsn.replace("512", "2000")
        ("json.json", "w").write(jsn)
        await bash("lottie_convert.py json.json pix.tgs")
        file = "pix.tgs"
        os.remove("json.json")
...
add_command_help(
    ...
    [
        ...
        [
            f"kang [emoji]",
            f"To add and customize the emoji sticker to your Sticker Pack.\n\n` • **NOTE:** To create a new Sticker Pack, use the numbers behind {cmd}kang\n • **EXAMPLE:** {cmd}kang 2",
        ],
        [
        ...
        ["stickers <name of sticker>", "For Searching for Sticker Packs."],
    ],
)
