...
from Pixsuvy.helpers.adminHelpers import DEVS
from Pixsuvy.helpers.basic import edit_or_reply
from Pixsuvy.helpers.misc import HAPP, XCB
from Pixsuvy.helpers.tools import get_arg
from Pixsuvy.utils.misc import restart
from Pixsuvy.utils.pastebin import PasteBin
from Pixsuvy.utils.tools import bash
...
async def upstream(client: Client, message: Message):
        ...
        if changelog:
            ...
            if len(changelog_str) > 4096:
                ...
                await client.send_document(
                    message.chat.id,
                    "output.txt",
                    caption=f"**Type** `{cmd}update deploy` **To Update Pixsuvy**",
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            repo.__del__()
            return
        await status.edit("`[HEROKU]: Update Deploy Pixsuvy is in progress...`")
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit("`Pixsuvy Successfully Updated! Userbot can be used again.`")
    else:
        try:
            ups_rem.pull(ac_br)
        except GitCommandError:
        ...
        await status.edit(
            "`Pixsuvy Successfully Updated! Userbot can be used again.`",
        )
...
add_command_help(
    "update",
    [
        ["update", "To see a list of the latest updates from Pixsuvy."],
        ["update deploy", "To update your Pixsuvy."],
    ],
)
