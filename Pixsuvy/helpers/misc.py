...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("Pixsuvy").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("Pixsuvy").info(f"Invalid Git Command")
        ...
        LOGGER("Pixsuvy").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("Pixsuvy").info(f"Heroku App Configured")
...
async def create_botlog(client):
    if HAPP is None:
        return
    LOGGER("Pixsuvy").info(
        "WAIT A MOMENT. CURRENTLY CREATING A USERBOT GROUP LOG FOR YOU"
    )
    ...
    try:
        ...
        LOGGER("ProjectMan").warning(
            "Your BOTLOG_CHATID var has not been filled. Create a telegram group and enter bot @MissRose_bot then type /id Enter the group id in var BOTLOG_CHATID"
        )
